...
interface NotificationSendModalProps {
  ...
  targetUserSid?: string | null; // 特定のユーザーに送信する場合
  onSuccess?: () => void;
}
...
export default function NotificationSendModal({
  ...
  const [selectedUserSids, setSelectedUserSids] = useState<Set<string>>(new Set());
  const [loading, setLoading] = useState(false);
  const [sending, setSending] = useState(false);
  const resolveUserId = (account?: UserType | null) => account?.id ?? account?.sid ?? null;
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.success) {
              ...
              if (targetUserSid) {
                fetchedUsers = fetchedUsers.filter((u: UserType) => resolveUserId(u) === targetUserSid);
              }
  ...
  }, [isOpen, authUserId, targetUserSid]);
  ...
  useEffect(() => {
    if (isOpen) {
      if (targetUserSid) {
        ...
        setSelectedUserSids(new Set([targetUserSid]));
      } else {
  ...
  }, [isOpen, material, targetUserSid]);

  const handleUserToggle = (userSid: string) => {
  ...
  const handleSelectAll = () => {
    if (selectedUserSids.size === users.length) {
      setSelectedUserSids(new Set());
    } else {
      const ids = users
        .map((u) => resolveUserId(u))
        .filter((id): id is string => Boolean(id));
      setSelectedUserSids(new Set(ids));
    }
  };

  const handleApplyFavorite = (userSids: string[]) => {
    setSelectedUserSids(new Set(userSids));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (material) {
        ...
        // ユーザー間のメッセージ
        const targetUser = users.find(u => resolveUserId(u) === targetUserSid);
      ...
      const response = await fetch('/api/notifications', {
        ...
        body: JSON.stringify({
          user_sids: Array.from(selectedUserSids),
          from_user_sid: authUserId,
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <button
              type="submit"
              disabled={sending || selectedUserSids.size === 0 || !message.trim()}
